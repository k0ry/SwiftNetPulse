...
**Язык:** [English](../../DEVELOPMENT.md) · [Русский](DEVELOPMENT.md)
...
Apple Swift 6.3, SDK macOS/iOS 26.4. После локализации: `swift test` 62 теста,
0 ошибок; `python3 -m unittest discover -s Tests/LocalizationChecks` 9 тестов,
0 ошибок. Debug- и release-сборки macOS, сборка iOS без подписи (в resource
bundle входят `en.lproj` и `ru.lproj`) и
`scripts/check-localization-consumer.sh` прошли успешно. Тесты на iOS-симуляторе
и физическом устройстве не запускались. Совместимость с минимальным Swift 5.9
...
Таблицы сообщений загружаются из `Bundle.module` (`en.lproj` и `ru.lproj`).
Вывод библиотеки по умолчанию — английский независимо от языка ОС. Отдельный
потребитель SwiftPM должен подтвердить загрузку ресурсов вне тестового таргета
этого пакета; см. `scripts/check-localization-consumer.sh`.

Чтобы добавить язык, скопируйте
`Sources/SwiftNetPulse/Resources/en.lproj/Localizable.strings` в новый `xx.lproj`,
переведите документацию в `docs/xx/` и зарегистрируйте язык в
[translations.json](../translations.json). Новый Swift API не требуется.
...
| `Localization` | Поиск строк для выбранного языка с запасным английским |
...
Приоритет дальнейшей работы (вне локализации): определить гарантии timeout/отмены
и потоков событий, добавить соответствующие тесты, затем подготовить переход на
Swift 6. Локализация не должна менять эти сетевые семантики.
